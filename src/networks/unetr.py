...
def build_unetr(cfg, keep_patch_embed: bool = False):
    if cfg.MODEL.ENCODER_CKPT_PATH != "":
        ...
        if not any([k.startswith('net.encoder') for k in state_dict.keys()]):
            raise ValueError(f"Checkpoint at {cfg.MODEL.ENCODER_CKPT_PATH} does not contain a compatible ViT encoder "
                             f"which's states are starting with 'net.encoder.'")
        if not any([k.startswith('net.encoder.patch_embedding') for k in state_dict.keys()]):
            raise ValueError(f"Checkpoint at {cfg.MODEL.ENCODER_CKPT_PATH} does not contain a compatible ViT encoder "
                             f"which's states are starting with 'net.encoder.patch_embedding'")
