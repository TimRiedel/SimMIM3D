...
class AdniDataset(CacheDataset):
    def __init__(
        ...
        if finetune_dxs is None:
            finetune_dxs = ['CN', 'AD']
        if pretrain_dxs is None:
        ...
        if section in ["train", "training"]:
            if is_pretrain:
                ...
                # Patients from the validation and test sets of fine-tuning must not be included
        ...
        phase = "pre-training" if is_pretrain else "fine-tuning"
