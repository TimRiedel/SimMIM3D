...
class BratsFinetuneData(pl.LightningDataModule):
    def __init__(
        ...
        assert torch.all(torch.tensor(self.input_size) <= torch.tensor(
            max_size)), "Not all dimensions of `input_size` are less than or equal to `(180, 180, 180)`"
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            if 0.0 < self.train_frac <= 1.0:
                ...
                raise ValueError(
                    f"Invalid value for `train_frac`: {self.train_frac}. `train_frac` must be in the range (0.0, 1.0].")
