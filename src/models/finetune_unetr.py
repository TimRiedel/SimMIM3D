...
class FinetuneUNETR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        loss, y_pred = self.common_step(batch)
    ...
    def on_validation_epoch_end(self):
        ...
        et_label = one_hot(torch.where(label == 3, 1.0, 0.0), num_classes=2)
        et_y_hat = one_hot(torch.where(y_hat == 3, 1.0, 0.0), num_classes=2)
        ...
        wt_label = one_hot(torch.where((label == 3) | (label == 2) | (label == 1), 1.0, 0.0), num_classes=2)
        wt_y_hat = one_hot(torch.where((y_hat == 3) | (y_hat == 2) | (y_hat == 1), 1.0, 0.0), num_classes=2)
        ...
        tc_label = one_hot(torch.where((label == 3) | (label == 2), 1.0, 0.0), num_classes=2)
        tc_y_hat = one_hot(torch.where((y_hat == 3) | (y_hat == 2), 1.0, 0.0), num_classes=2)
