...
class PretrainSimMIM(LightningModule):
    def __init__(
            self,
            net,
            learning_rate: float,
    ...
    def common_step(self, x, mask):
        ...
        mask = mask.repeat_interleave(patch_size[0], 1).repeat_interleave(patch_size[1], 2).repeat_interleave(
            patch_size[2], 3).unsqueeze(1).contiguous()  # type: ignore
        x_masked = x * (1 - mask)
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        self.log("training/loss", outputs["loss"], on_step=False, on_epoch=True, sync_dist=True,
                 batch_size=batch["image"].shape[0])  # type: ignore
    ...
    def on_validation_batch_end(self, outputs, batch, batch_idx, **kwargs):
        self.log("validation/loss", outputs["loss"], on_step=False, on_epoch=True, sync_dist=True,
        ...
        if batch_idx == 0:
            ...
            if self.current_epoch == 0:
                ...
                self.logger.experiment.log(
                    {"validation/original": [wandb.Image(img) for img in images]})  # type: ignore
            ...
            self.logger.experiment.log(
                {"validation/reconstruction": [wandb.Image(recon) for recon in reconstructions]})  # type: ignore
